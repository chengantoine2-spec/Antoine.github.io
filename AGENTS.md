...
- 评论/点赞：Giscus（挂载 GitHub Discussions）
...
- 访客：读博客、Giscus 登录后评论点赞
- 站长：创建/编辑/删除 Issue、上传图片、改外观
- 管理操作走 PAT，存 localStorage，不提交仓库；非登录态隐藏 /write 入口
...
src/lib/github.ts          // GitHub API 封装（Issues、上传图片）
src/lib/giscus.ts          // Giscus 配置与挂载
src/lib/markdown.ts        // react-markdown 配置

src/hooks/useBlogs.ts
src/hooks/useAuth.ts       // PAT 登录态
...
- 仓库开启 Discussions
- 博客 Issue 与 Discussion 通过标题或 number 映射
- Giscus 主题跟随站点亮暗色切换（用 setConfig 动态换主题）
